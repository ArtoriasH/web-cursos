<?php
    ...
    $dataInstructorCourses = $tempData->bringSoldCourses($_SESSION['id']);
    
    //print_r($dataInstructorCourses);
?>
...
<head>
    ...
    <body>
        ...
        <div class="contenedor">
            ...
            <div style="margin-top: 26px;">
                <div class="container">
                    <div class="row">  
                        <?php
                            foreach($dataInstructorCourses as $rowInstructorCourses){
                        ?>
                                <div>
                                    <a href="curso.php?idCourse=<?php echo($rowInstructorCourses["idCurso"])?>" style='text-decoration: none;font-weight: bold;color: #5d1b8f'>
                                        <h2 class="card-text"><?php echo $rowInstructorCourses['nombreCurso'] ?></h2>
                                    </a>
                        <?php

                                    $dataCourseStudents = $tempData->bringCourseStudents($rowInstructorCourses['idCurso']);
                                    if($rowInstructorCourses["contenidoP"] == 1){
                                        $dataCourseProfit = $tempData->bringCourseProfit($rowInstructorCourses['idCurso']);
                                    }else{
                                        $dataCourseProfit = $tempData->bringLevelProfit($rowInstructorCourses['idCurso']);
                                    }
                                    echo "<p> Ganancia obtenida en este curso: $".$dataCourseProfit['totalCurso']."</p>";
                                    foreach($dataCourseStudents as $rowCourseStudents){
                        ?>
                                        <div class="gray-container-wth-Clmns">                                                                  
                                            <div class="card-body" style="padding: 1% 0 1% 1%; margin-left: 1%;">
                                                <p class="card-text" style="font-weight: bold;"><?php echo $rowCourseStudents['nombre'] ?></p> 
                                                <p class="card-text">
                                                    Fecha ingreso: <?php $date = date_create($rowCourseStudents['fechaAdquirido']); echo date_format($date,"d/M/Y");?>
                                                </p> 
                                                <p class="card-text">
                                                    Metodo de pago: <?php if($rowCourseStudents['metodoPago']==0){echo "PayPal";}else{echo "Pago a tarjeta";}?>
                                                </p>
                                                <?php
                                                    if($rowCourseStudents["contenidoP"] == 1){
                                                        echo "<p class='card-text'>Precio con el que se adquirio el curso: $".$rowCourseStudents['precioOriginal']."</p>";
                                                    }else{
                                                        $dataLevels = $tempData->bringLevelsCourse($rowInstructorCourses['idCurso']);
                                                        foreach($dataLevels as $rowDataLevels){
                                                            $dataOriginalLvlPrice = $tempData->bringOriginalLvlPrice($rowCourseStudents["alumno"], $rowDataLevels["idNivel"]);
                                                            //print_r($dataOriginalLvlPrice);
                                                            if(!empty($dataOriginalLvlPrice)){
                                                                echo "<p class='card-text'>Compró: ".$dataOriginalLvlPrice['nombreNiv']." a: $".$dataOriginalLvlPrice['precioOriginal']."</p>";
                                                            }else{
                                            ...
                                            <div class="card-body"style="padding: 1% 0 1% 1%; margin-left: 1%;">
                                                <?php 
                                                    if($rowCourseStudents['estadoCompleto'] == 1){
                                                        if($rowCourseStudents['diploma'] == 0){
                                                ?>
                                                        <form action="includes/actions.php" method="POST">
                                                            <input type="hidden" name="idVenta" value="<?php echo $rowCourseStudents['venta'] ?>">                                            
                                                            <button class='btn btn-primary' name="btnDarDiploma" type="submit" style="background:#6d28aa; border:none; color:white; text-align:center;">
                                                                Dar diploma
                                                            </button>
                                                        </form>
                                                <?php
                                                            ...
                                                            echo "Esta persona ya completo su curso y ya tiene su diploma";
                                                        }
                                                    }
                                                ?>
                                            </div>
        ...
        <!-- Footer -->
        <footer class="text-center bg-dark footer" style="margin-top: 160px">
