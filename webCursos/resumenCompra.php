...
session_start();
    ...
    if(isset($_GET['idNivel'])){
        ...
        $dataCourse = $tempData->bringCourse($dataLevel['idCurso']);
    }
    //print_r($dataLevel);
?>
...
<html lang="en">
    ...
    <body>
        ...
        <div class="contenedor" style="display:flex; justify-content: center;">
            <div class="gray-container-center">
                <h1>Resumen de tu compra:</h1>
                <h2>Curso: "<?php echo $dataCourse['nombreCurso']?>"</h2>
                <?php
                if(isset($_GET['idCourse'])){
                    echo '<p>Precio del curso: $' . $dataCourse['precio']."</p>";
                }
                if(isset($_GET['idNivel'])){
                    echo '<p>nivel: ' . $dataLevel['nombreNiv']."</p>";
                    if($dataLevel['gratuito'] == 1){
                        echo '<p>Precio del nivel: $0</p>';
                    }else{
                        echo '<p>Precio del nivel: $' . $dataLevel['levelPrice']."</p>";
                    }
                } 
                ?>
                <div>
                    <form action="includes/actions.php" method="POST">
                        ...
                        <?php
                            if(isset($_GET['idCourse'])){
                                echo '<input type = "hidden" name = "precioOriginal" value= "'. $dataCourse["precio"].'" > ';
                                echo '<input type="hidden" name="idCurso" value="'. $dataCourse["idCurso"].'">';
                            }

                            if(isset($_GET['idNivel'])){
                                if($dataLevel['gratuito'] == 1){
                                    echo '<input type="hidden" name="precioOriginal" value="0">';
                                }else{
                                    echo '<input type = "hidden" name = "precioOriginal" value= "'. $dataLevel["levelPrice"].'" > ';
                                }
                                echo '<input type="hidden" name="idNivel" value="'. $dataLevel["idNivel"].'">';
                            }   
                        ?>
                        <button type="submit" name="btnComprar" style="background:#6d28aa; border:none; border-radius:5px; height:35px; color:#fff; margin-bottom:15%;">
                            Comprar
                        </button>
                    </form>
                </div>
        ...
        <!-- Footer -->
        <footer class="text-center bg-dark footer" style="margin-top: 500px;">
