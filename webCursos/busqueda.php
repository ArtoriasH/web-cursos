<?php
    ...
    if(!isset($_GET['dateFrom']) && !isset($_GET['dateTo'])){
        $dataSearch = $tempData->bringSearch($_GET['kWord']);
    }else{
        $dataSearchDateFilters = $tempData->bringSearchDateFilters($_GET['kWord'], $_GET['dateFrom'], $_GET['dateTo']);
    }
...
<html lang="en">
    <head>
        ...
        <!-- Titulo -->
        <title>Resultados de: <?php echo $_GET['kWord']?></title>
        <!-- Favicon -->
        ...
        <div class="contenedor">
                ...
                <div class="row d-xxl-flex">
                    <div class="col">
                        <h1 class="d-xxl-flex justify-content-xxl-center align-items-xxl-center" style="color:#fff; padding-top:15px;">
                            Resultados de: <?php echo $_GET['kWord']?>
                        </h1>
                    </div>
                </div>
            </div>
            <div style="display: flex;justify-content:center; padding: 1%;">
                <form method="get" action="busqueda.php?">
                    <input type="hidden" name="kWord" value="<?php echo $_GET['kWord']?>">
                    <lable>De:</label>
            ...
            <div style="margin-top: 26px;">
                <div class="container">
                    <div class="row" style="display:flex; flex-wrap:wrap; justify-content:space-evenly;">
                        <?php
                            if(isset($dataSearch)){
                                foreach($dataSearch as $rowSearch){
                        ?>
                                <div style="height:15%; width:15%; background-color:#f2f2f2; margin:1%; padding:1%; border-radius:5px; display:flex; flex-direction:column; justify-content:center; align-items:center;">                
                                    <a href="curso.php?idCourse=<?php echo($rowSearch["idCurso"])?>">
                                        <img src="data:image/jpeg/png;base64,<?php echo base64_encode($rowSearch['imagen'])?>" style="height:150px; width:150px; border-radius:5px;">
                                    </a>
                                    <div >
                                        <a href="curso.php?idCourse=<?php echo($rowSearch["idCurso"])?>" style="text-decoration:none; font-weight:bold; color:#5d1b8f">
                                            <p class="card-text"><?php echo($rowSearch["nombreCurso"])?></p>
                                        </a>
                                    </div>
                                </div>
                        <?php
                                ...
                                foreach($dataSearchDateFilters as $rowSearch){
                        ?>
                                <div style="height:15%; width:15%; background-color:#f2f2f2; margin:1%; padding:1%; border-radius:5px; display:flex; flex-direction:column; justify-content:center; align-items:center;">                
                                    <a href="curso.php?idCourse=<?php echo($rowSearch["idCurso"])?>">
                                        <img src="data:image/jpeg/png;base64,<?php echo base64_encode($rowSearch['imagen'])?>" style="height:150px; width:150px; border-radius:5px;">
