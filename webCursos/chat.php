<?php
    ...
    $dataMessages = $tempData->bringMessages($_GET['idChat']);
    $dataChat = $tempData->bringChat($_GET['idChat']);
...
<html lang="en">
    ...
    <body>
            ...
            <div class="bootstrap_chat">
                <div class="container py-5 px-4">
                    <header class="text-center">
                        <?php 
                        if($dataChat['alumno'] == $_SESSION['id']){
                            echo "<h1 class='display-4 text-white'>Maestro: " .  $dataChat['nombreI'] . "</h1>";
                        }else{
                            echo "<h1 class='display-4 text-white'>Alumno: " .  $dataChat['nombreA'] . "</h1>";
                        } ?>
                        
                    </header>
                    <div class="row rounded-lg" style="display:flex; justify-content: center;"><!-- Chat Box-->
                        <div style="width:60%; ">
                            <div class="px-4 py-5 chat-box bg-white">        
                                <?php 
                                    foreach($dataMessages as $rowMessages){
                                        if($rowMessages['remitente'] == $_SESSION['id']){
                                ?>
                                        <!-- Reciever Message-->
                                        <div class="media w-50 ml-auto mb-3" style="position: relative;left: 350px;">
                                            <div class="media-body">
                                                <div class="bg-primary rounded py-2 px-3 mb-2" style="background-color: #673296;">
                                                    <p class="text-small mb-0 text-white"> <?php echo $rowMessages['contenidoMsg'] ?></p>
                                                </div>
                                                <p class="small text-muted"><?php echo $rowMessages['hora'] ?></p>
                                            </div>
                                        </div>    
                                <?php
                                        }
                                ...
                                ?>
                                        ...
                                        <?php 
                                        if($dataChat['alumno'] == $_SESSION['id']){
                                            echo "<img src='data:image/jpeg/png;base64," .  base64_encode($dataChat['fotoI']) . "' alt='user' width='50' height='50' class='rounded-circle'>";
                                        }else{
                                            echo "<img src='data:image/jpeg/png;base64," .  base64_encode($dataChat['fotoA']) . "' alt='user' width='50' height='50' class='rounded-circle'>";
                                        } 
                                        ?>
                                            
                                            <div class="media-body ml-3">
                                                <div class="bg-light rounded py-2 px-3 mb-2">
                                                    <p class="text-small mb-0 text-muted"> <?php echo $rowMessages['contenidoMsg'] ?></p>
                                                </div>
                                                <p class="small text-muted"> <?php echo $rowMessages['hora'] ?></p>
                                            </div>
                                        </div>
                            ...
                            <!-- Typing area -->
                            <form action="includes/actions.php" method="POST" class="bg-light">
                                <input type="hidden" name="remitente" value="<?php echo $_SESSION['id']?>">
                                <input type="hidden" name="idChat" value="<?php echo $dataChat['idChat']?>">
                                <div class="input-group">
                                    <input type="text" name="contenidoMsg" placeholder="Type a message" aria-describedby="button-addon2" class="form-control rounded-0 border-0 py-4 bg-light">
                                    <div class="input-group-append">
                                        <button type="submit" name="btnSendMessage" class="btn btn-link"><i class="fa fa-paper-plane"></i></button>
                                    </div>
                                </div>
                            </form>
                        </div>
        ...
        <!-- Footer -->
        <footer class="text-center bg-dark footer" style="margin-top:0">
